...
describe('Code', () => {
    describe('separateDigits', () => {
        ...
        test('should throw error on more than two digit numbers', () => {
            expect(() => separateDigits(100)).toThrowError();
        });
    });
    ...
    describe('getCharacter', () => {
        test('should get correct character', () => {
            ...
            expect(getCharacter(grid, { x: 0, y: 0 })).toBe('A');
            expect(getCharacter(grid, { x: 1, y: 0 })).toBe('B');
            expect(getCharacter(grid, { x: 2, y: 0 })).toBe('C');
            expect(getCharacter(grid, { x: 0, y: 1 })).toBe('D');
            expect(getCharacter(grid, { x: 1, y: 1 })).toBe('E');
            expect(getCharacter(grid, { x: 2, y: 1 })).toBe('F');
        });
        ...
        test('should throw error if outside grid', () => {
            ...
            expect(() => getCharacter(grid, { x: 4, y: 4 })).toThrowError();
            expect(() => getCharacter(grid, { x: -4, y: 4 })).toThrowError();
            expect(() => getCharacter(grid, { x: 4, y: -4 })).toThrowError();
            expect(() => getCharacter(grid, { x: -4, y: -4 })).toThrowError();
        });
    ...
    describe('getCode', () => {
        ...
        test('should throw error if not from alphabet', () => {
            expect(() => getCode(grid, 'LO', 'R')).toThrowError();
            expect(() => getCode(grid, 'L', '?')).toThrowError();
        });
    });
